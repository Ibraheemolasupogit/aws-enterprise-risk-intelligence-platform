"""Train and evaluate a baseline local churn prediction model."""
...
CHURN_REPORT_PATH = Path("reports/churn_model_report.md")
...
def select_model_features(dataset: pd.DataFrame) -> list[str]:
    """Select numeric, model-ready churn features while avoiding leakage."""
...
def assign_churn_risk_band(probability: float) -> str:
    """Convert churn probability into a simple risk band."""
    if probability >= 0.60:
        return "High"
    if probability >= 0.30:
        return "Medium"
...
def calculate_lift_at_top_decile(y_true: pd.Series, probabilities: pd.Series) -> float:
    """Calculate churn lift among the highest-scored 10 percent of records."""
    scored = pd.DataFrame({"churn_label": y_true.to_numpy(), "churn_probability": probabilities.to_numpy()})
    top_decile_size = max(1, int(len(scored) * 0.10))
    top_decile = scored.sort_values("churn_probability", ascending=False).head(top_decile_size)
    baseline_rate = scored["churn_label"].mean()
    if baseline_rate == 0:
        return 0.0
    return round(float(top_decile["churn_label"].mean() / baseline_rate), 4)
...
def train_churn_model(
    ...
    """Train and evaluate a baseline churn classifier."""
    ...
    probability_series = pd.Series(probabilities, index=y_test.index)
    ...
    metrics = {
        ...
        "lift_at_top_decile": calculate_lift_at_top_decile(y_test, probability_series),
        "confusion_matrix": {
    ...
    prediction_output["churn_prediction"] = predictions
    prediction_output["churn_probability"] = probabilities.round(6)
    prediction_output["churn_risk_band"] = prediction_output["churn_probability"].apply(assign_churn_risk_band)
    prediction_output = prediction_output.sort_values("churn_probability", ascending=False)
...
def build_churn_model_report(metrics: dict[str, Any]) -> str:
    """Build a Markdown report for the churn model."""
    ...
    lines = [
        ...
        f"- Positive churn class rate: {metrics['positive_class_rate']:.1%}",
        ...
        f"- Lift at top decile: {metrics['lift_at_top_decile']:.4f}",
        "",
        "## Confusion Matrix",
        "",
        "| Metric | Count |",
...
def write_churn_model_artifacts(
    dataset_path: str | Path = FEATURE_DATASET_PATH,
    metrics_path: str | Path = CHURN_METRICS_PATH,
    predictions_path: str | Path = CHURN_PREDICTIONS_PATH,
    report_path: str | Path = CHURN_REPORT_PATH,
) -> dict[str, Path]:
    """Train the churn model and write local evidence artifacts."""
    result = train_churn_model(dataset_path=dataset_path)
    metrics = result["metrics"]
    predictions = result["predictions"]
    report = build_churn_model_report(metrics)
...
def main() -> None:
    """CLI entry point for local churn model training."""
    artifacts = write_churn_model_artifacts()
    for artifact_name, artifact_path in artifacts.items():
        print(f"Wrote churn {artifact_name}: {artifact_path}")
